...
if chat_info == 'block' then 
 api.sendMessage(msg.chat.id, '`- Sorry !`\n_شما بلاک شده اید_', true) 
else
 db:hset(hash, user_id, 'true')
 api.sendMessage(msg.chat.id, '*- چت آغاز شد*\n `هم اکنون میتونید چت کنید`\n\n_لطفا پیام خود را ارسال نمایید!_', true) 
 end
 end
...
api.sendMessage(msg.chat.id, '*- چت پایان یافت* \n/start برگشت به صفحه اول', true) 
end
end
if msg.chat.type == 'private' and chat_info == 'true' then
if blocks[1] == 'end' or blocks[1] == 'chat' then return nil end
api.forwardMessage('-1001070258140', msg.chat.id, msg_id) 
api.sendKeyboard(msg.chat.id, '`- پیام شما ارسال شد !`\n_لطفا منتظر پاسخ بمانید_'  ,do_keyboard_endchat(), true)
end
if blocks[1] == 'block' then
if msg.reply and msg.reply.forward_from and msg.chat.type == 'supergroup' and msg.chat.id == -1001070258140 and not blocks[2] then
msg = msg.reply
...
api.sendMessage(user_id, '`- متاسفیم`\n_شما بلاک شدهید_\n\n*چت پایان یافت.*', true) 
 ...
 if msg.chat.type == 'supergroup' and msg.chat.id == -1001070258140 then
...
api.sendMessage(user_id, '_You are UnBlocked_', true) 
    ...
    if msg.reply and msg.reply.forward_from and msg.chat.type == 'supergroup' and msg.chat.id == -1001070258140 then
